...
export default function DynamicPricing({ company }) {
  ...
  async function loadDynamicPrices() {
    ...
    try {
      ...
      res.data.forEach(dp => {
        priceMap[dp.product_id] = dp.price
        inputMap[dp.product_id] = dp.price.toString()
      })
  ...
  const handleSavePrice = async (productId) => {
    ...
    try {
      if (!inputValue || inputValue === "") {
        ...
        await setDynamicPrice(selectedPartyId, productId, price)
        setDynamicPrices(prev => ({
          ...prev,
          [productId]: price
        }))
  ...
  return (
    <div className="max-w-[1200px] mx-auto px-6 py-6 font-display">
      ...
      <div className="bg-white border border-[#dce4e5] rounded-xl p-4 shadow-sm mb-6">
        ...
        <div className="relative max-w-md party-dropdown">
          ...
          <input
            ...
            placeholder="Search party by name / code / phone..."
            className="w-full h-12 rounded-lg border border-[#dce4e5] px-4 text-sm focus:ring-2 focus:ring-primary/50 outline-none"
          />

          {/* DROPDOWN */}
          {showDropdown && (
            <div className="absolute z-50 mt-1 w-full bg-white border border-[#dce4e5] rounded-lg shadow-lg max-h-60 overflow-auto">
              ...
              {filteredParties?.length === 0 ? (
                ...
                filteredParties?.map(p => (
                  <div
                    ...
                    className={`px-4 py-2 text-sm cursor-pointer hover:bg-primary/10 flex flex-col ${selectedPartyId === p.id ? "bg-primary/5" : ""
                      }`}
                  >
                    <span className="font-medium text-[#111718]">{p.name}</span>
                    <span className="text-xs text-[#638288]">
                      {p.party_code} • {p.phone || "No phone"}
                    </span>
                  </div>
                ))
              )}
